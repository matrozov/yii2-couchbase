...
class QueryBuilder extends Object
{
     ...
     */
    public function insert($bucket, $columns, &$params)
    {
        $names = [];
        $placeholders = [];
        ...
        if ($columns instanceof Query) {
            list($names, $values, $params) = $this->prepareInsertSelectSubQuery($columns);
        }
        else {
        ...
        return 'INSERT INTO ' . $this->db->quotebucketName($bucket)
     ...
     */
    protected function prepareInsertSelectSubQuery($columns, $params = [])
    {
     ...
     */
    public function batchInsert($bucket, $rows)
    {
        if (empty($rows)) {
            return '';
        }
